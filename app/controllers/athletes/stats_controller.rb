class Athletes::StatsController < ApplicationController
  ...
  def update
    ...
    if @stat.save
      redirect_to edit_athlete_stats_path(@athlete, position: @stat.position.position_name, season: @stat.position.season)
    else
  ...
  def destroy
    if @athlete.position(params[:id]).all.destroy_all
      ...
      respond_to do |format|
        format.js
        format.html {redirect_to edit_athlete_stats(@athlete) }
      end
    end
  end
